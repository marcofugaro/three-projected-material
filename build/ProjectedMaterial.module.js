import { LogLuvEncoding, GammaEncoding, RGBDEncoding, RGBM16Encoding, RGBM7Encoding, RGBEEncoding, sRGBEncoding, LinearEncoding, MeshPhysicalMaterial, Vector2, Matrix4, Vector3, InstancedBufferAttribute } from 'three';
...
class ProjectedMaterial extends MeshPhysicalMaterial {
  constructor({
    ...
    this.uniforms = {
      projectedTextures: { value: textures },
      ...
      viewMatricesCamera: { value: textures.map(() => new Matrix4()) },
      projectionMatrixCamera: { value: new Matrix4() },
      projPositions: { value: textures.map(() => new Vector3()) },
      projDirections: { value: textures.map(() => new Vector3(0, 0, -1)) },
      // we will set this later when we will have positioned the object
      savedModelMatrices: { value: textures.map(() => new Matrix4()) },
...
function project(mesh, { textureIndex: i = 0 } = {}) {
...
function projectInstanceAt(
  ...
  if (
    !instancedMesh.geometry.attributes[`savedModelMatrix0_${i}`] ||
    !instancedMesh.geometry.attributes[`savedModelMatrix1_${i}`] ||
    !instancedMesh.geometry.attributes[`savedModelMatrix2_${i}`] ||
    !instancedMesh.geometry.attributes[`savedModelMatrix3_${i}`]
  ) {
    throw new Error(
  ...
  instancedMesh.geometry.attributes[`savedModelMatrix0_${i}`].setXYZW(
    index,
  ...
  instancedMesh.geometry.attributes[`savedModelMatrix1_${i}`].setXYZW(
    index,
  ...
  instancedMesh.geometry.attributes[`savedModelMatrix2_${i}`].setXYZW(
    index,
  ...
  instancedMesh.geometry.attributes[`savedModelMatrix3_${i}`].setXYZW(
    index,
